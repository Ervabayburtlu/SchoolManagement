...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequestDto request)
    {
        ...
        if (!captchaValid)
            return BadRequest(ApiResponse<object>.ErrorResponse("Captcha doğrulaması başarısız."));
        ...
        if (result == null)
            return Unauthorized(ApiResponse<object>.ErrorResponse("E-posta veya parola hatalı."));
    ...
    public async Task<IActionResult> ObsLogin([FromBody] LoginRequestDto request)
    {
        try
        {
            ...
            if (result == null)
                return Unauthorized(ApiResponse<object>.ErrorResponse("E-posta veya parola hatal�."));
        ...
        {
            return StatusCode(500, ApiResponse<object>.ErrorResponse("OBS giriş işlemi sırasında sunucu hatası oluştu."));
        }
    }

    // Access token s�resi doldu�unda yeni token almak i�in
    [HttpPost("refresh")]
    public async Task<IActionResult> Refresh([FromBody] RefreshRequestDto request)
    {
        ...
        {
            return BadRequest(ApiResponse<object>.ErrorResponse("UserId, Role ve RefreshToken alanları zorunludur."));
        }

        var result = await _authService.RefreshTokenAsync(request);

        if (result == null)
            return Unauthorized(ApiResponse<object>.ErrorResponse("Geçersiz veya süresi dolmuş refresh token."));

        return Ok(ApiResponse<object>.SuccessResponse(result, "Token refreshed successfully"));
    }

    // Refresh token'� ge�ersizle�tirir (logout)
    [HttpPost("logout")]
    [Authorize]
